...
public class Move : MonoBehaviour {
    ...
    void Update()
    {
        if (moving_status == 1)
        {
            transform.position = Vector3.MoveTowards(transform.position, middle, 20 * Time.deltaTime);
        ...
        {
            transform.position = Vector3.MoveTowards(transform.position, dest, 20 * Time.deltaTime);
